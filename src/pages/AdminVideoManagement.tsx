...
const CrudModal: React.FC<CrudModalProps> = ({ isOpen, onClose, videoToEdit, onSave, isSaving, tours }) => {
  ...
  const validateForm = () => {
    ...
    // Validate File (chỉ bắt buộc khi thêm mới)
    if (!isEditMode && !file) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Gọi validation trước khi xử lý
    if (!validateForm()) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-xl border transition-all outline-none bg-gray-50/50 focus:bg-white
                ${errors.title 
            ...
            />
            {errors.title && <p className="text-red-500 text-xs mt-1 font-medium animate-pulse">{errors.title}</p>}
          </div>
          ...
          <div>
            <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-300 group 
               ...
               ${file ? 'bg-green-50 border-green-400' : 'hover:bg-purple-50 hover:border-purple-300 border-gray-200'}`}
            >
              <label className="cursor-pointer block w-full h-full">
                ...
                <input type="file" accept="video/*" onChange={(e) => {
                  ...
                  if (e.target.files?.[0]) setErrors((prev) => ({ ...prev, file: undefined }));
                }} className="hidden" disabled={isSaving || isUploading} />
              </label>
            </div>
            {errors.file && <p className="text-red-500 text-xs mt-1 font-medium text-center animate-pulse">{errors.file}</p>}
          </div>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-9 h-9 flex items-center justify-center rounded-lg text-sm font-bold transition-all ${currentPage === number
