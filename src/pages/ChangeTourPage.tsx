...
import { getTours, searchTours } from "../services/tour.service";
...
const ChangeTourPage = () => {
    ...
    // Pagination Logic
    const totalPages = useMemo(() => Math.ceil(tours.length / TOURS_PER_PAGE), [tours]);
    ...
    const calculatePriceDiff = () => {
        ...
        const newTotal = selectedTour.priceAdult * oldBooking.numOfAdults + selectedTour.priceChild * oldBooking.numOfChildren;
            ...
            <Container maxWidth="xl">
                                    ...
                                    <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                                        <Box>
                                            <Box p={2} bgcolor="#f8fafc" borderRadius={2}>
                                                ...
                                                <Typography fontWeight={600}>{oldBooking.tourTitle}</Typography>
                                        ...
                                        <Box>
                                            <Box p={2} bgcolor="#eff6ff" borderRadius={2} border="1px solid #bfdbfe">
                                                <Typography variant="subtitle2" color="primary" mb={1}>
                                                    Tour mới
                                                </Typography>
                                                <Typography fontWeight={600}>{selectedTour.title}</Typography>
                                                <Typography variant="body2" color="primary">
                                                    {formatCurrency(selectedTour.priceAdult * oldBooking.numOfAdults + selectedTour.priceChild * oldBooking.numOfChildren)} (Dự kiến)
                                                </Typography>
                                            </Box>
                                        </Box>
                                    </Box>

                                    <Box mt={3} p={2} bgcolor="#fff7ed" borderRadius={2} display="flex" gap={2}>
                            ...
                            <Card sx={{ borderRadius: 4, position: "sticky", top: 20 }}>
                                <CardContent sx={{ p: 3 }}>
                                    ...
                                    <Box display="flex" justifyContent="space-between" mb={1}>
                                        ...
                                        <Typography fontWeight={600} color={calculatePriceDiff() > 0 ? "error.main" : "success.main"}>
                                    ...
                                    >
                                        {isSubmitting ? <CircularProgress size={24} color="inherit" /> : "Xác nhận đổi tour"}
                                    </Button>
                                    <Button
                                        fullWidth
                                        variant="text"
                ...
                {activeStep === 2 && (
                    <Box textAlign="center" py={5}>
                        ...
                        <div id="embedded-payment" style={{ height: "400px", width: "100%" }}></div>
                        {paymentStatus === "failed" && (
                            <Button onClick={() => handleConfirmChange()} variant="contained" color="error" sx={{ mt: 2 }}>
                                Thử lại
                            </Button>
                        )}
                    </Box>
                ...
                {activeStep === 3 && (
                    <Box textAlign="center" py={8}>
                        ...
                        <Typography color="text.secondary" mb={4} maxWidth="sm" mx="auto">
                            Yêu cầu đổi tour của bạn đã được gửi đến hệ thống.
